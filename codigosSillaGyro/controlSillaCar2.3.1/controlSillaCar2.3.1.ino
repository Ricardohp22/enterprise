...
void leerJoystick(boolean bluetooth)
{
  if (bluetooth)
  {
    ...
    //Multiplicador de velocidad al frente
    if(yy_ > 0){
      yy_ = yy_*2;
    }
    /* Serial.print(xx_);
...
int nuevoPwmI = 0;
int nuevoPwmD = 0;
boolean flagSensorActivado = 0;
boolean flagBloqueoFrontal = 0;
boolean flagBloqueoTrasero = 0;
boolean flagBloqueoDerecha = 0;
boolean flagBloqueoIzquierda = 0;
boolean velocidadBajaActivada = 0;
unsigned long timeLiberarBloqueo = 0;

unsigned long timeLiberarBloqueoD = 0;
unsigned long timeLiberarBloqueoF = 0;
unsigned long timeLiberarBloqueoI = 0;
unsigned long timeLiberarBloqueoT = 0;
void bloqueoMotoresPorSensor()
{
  //Bloqueo de motores por activacion de sensores de distancia
  //bloqueo frontal
  if (s1 || s2 || s5 || s6 || s7 || s8 || s9 || s10)
  {
    flagSensorActivado = 1;
    timeLiberarBloqueo = millis();
  }
  else
  {
    flagSensorActivado = 0;
    if (((millis() - timeLiberarBloqueo) > 5000) && cabezaNeutral)
    {
      velocidadBajaActivada = 0;
    }
  }

  if (s1 || s2 || s5 || s6)
  {
    flagBloqueoFrontal = 1;
    timeLiberarBloqueoF = millis();
    //Serial.println("bloqueo frontal");
  }
  else
  {
    if (((millis() - timeLiberarBloqueoF) > 1500) && cabezaNeutral)
    {
      flagBloqueoFrontal = 0;
    }
  }
  //bloqueo trasero
  if (s3 || s4 || s7 || s8)
  {
    timeLiberarBloqueoT = millis();
    flagBloqueoTrasero = 1;

    //Serial.println("bloqueo trasero");
  }
  else
  {
    if (((millis() - timeLiberarBloqueoT) > 1500) && cabezaNeutral)
    {
      flagBloqueoTrasero = 0;
    }
  }
  //bloqueo izquierda
  if (s9)
  {
    timeLiberarBloqueoI = millis();
    flagBloqueoIzquierda = 1;

    //Serial.println("bloqueo izquierda");
  }
  else
  {
    if (((millis() - timeLiberarBloqueoI) > 1500) && cabezaNeutral)
    {
      flagBloqueoIzquierda = 0;
    }
  }
  //bloqueo derecha
  if (s10)
  {
    timeLiberarBloqueoD = millis();
    flagBloqueoDerecha = 1;

    //Serial.println("bloqueo derecha");
  }
  else
  {
    if (((millis() - timeLiberarBloqueoD) > 1500) && cabezaNeutral)
    {
      flagBloqueoDerecha = 0;
    }
  }

  if ((flagBloqueoFrontal || flagBloqueoIzquierda || flagBloqueoTrasero || flagBloqueoDerecha) && (cabezaNeutral))
  {
    velocidadBajaActivada = 1;
  }

  if (velocidadBajaActivada == 0)
  {
    if (flagBloqueoFrontal)
    {
    ...
    if (flagBloqueoIzquierda)
    {
      pwm_derecha = 0;
      pwm_izquierda = pwm_izquierda/3;
    }
    if (flagBloqueoDerecha)
    {
      pwm_izquierda = 0;
      pwm_derecha = pwm_derecha/3;
    }
  }
  else
  {
